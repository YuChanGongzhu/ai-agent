...
export const WxMpDialogPage: React.FC<WxMpDialogPageProps> = ({
  ...
  const fetchMessages = async (reset = false) => {
    ...
    try {
      ...
      if (response.code === 0) {
        const newMessages = response.data.records.map(
          (msg: MpChatMessage): Message => ({
            id: msg.msg_id,
      ...
      >
        {isLoading && page === 0 ? (
          ...
          <>
            ...
            {messages.length === 0 ? (
              <div className="text-center py-10 text-gray-500">暂无消息记录</div>
            ) : (
              <div className="space-y-4">
                {messages.reverse().map((message, index) => (
                  ...
                  >
                    <div
                      className={`max-w-[80%] ${
                        message.isUser ? "border border-[#498e61]" : "bg-green-100"
